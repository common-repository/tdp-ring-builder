...
const getters = {
  diamondData: (state) => state.diamondData,
...
const actions = {
  async fetchDiamondDetail({ commit, state }, { id, type }) {
    ...
    data.append('action', apiActions.diamondDetailsApi);
    !isDevelopmentMode && data.append("nonce", tdprbajax.nonce);
    data.append('diamond_type', type);
    data.append('sku', id);
    ...
    try {
      ...
      if (response.data.data.success) {
        const diamondData = response.data.data.data || {};
        sessionStorage.setItem('tdpx-diamond-meta-data', JSON.stringify(diamondData));
        commit('SET_DIAMOND_DATA', diamondData);
        commit('SET_DATA_NOT_FOUND', { dataNotFound: false });
      } else {
        commit('SET_DIAMOND_DATA', {});
