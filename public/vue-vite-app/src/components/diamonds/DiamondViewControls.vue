<template lang="">
    ...
    <div class='tdprb-w-full tdprb-h-full tdprb-flex tdprb-flex-col md:tdprb-flex-row tdprb-gap-[20px] tdprb-justify-between tdprb-items-center tdprb-px-[15px] md:tdprb-px-[30px] md:tdprb-py-10 tdprb-py-5  tdprb-border-b tdprb-border-borderColor'>
        <div class='tdprb-w-full tdprb-h-full tdprb-flex tdprb-justify-center tdprb-items-start tdprb-flex-col tdprb-gap-2 '>
           <span class="tdprb-font-lato md:tdprb-text-[22px] tdprb-text-[18px] tdprb-font-[500] tdprb-leading-[20px] md:tdprb-leading-[30px] tdprb-text-themeColor">{{`${toSentenceCase(diamondShape)} ${toSentenceCase(diamondType)} Diamond`}}</span>
           <span class="tdprb-font-lato md:tdprb-text-[14px] tdprb-font-[400] md:tdprb-leading-[16.8px] tdprb-text-[12px] tdprb-leading-[14.8px] tdprb-text-textSecondaryColor">{{`Shop Conflict-Free Certified ${toSentenceCase(diamondType)} Diamonds`}}</span>
        </div>
...
<div v-if="Number(getListView) || Number(getGridView) || 0"
    class='tdprb-min-w-[10px] tdprb-h-[50px]  tdprb-flex tdprb-justify-center tdprb-items-center tdprb-gap-1 tdprb-p-1 tdprb-border tdprb-border-themeColor tdprb-rounded tdprb-bg-backgroundColor'>
    <button v-if="Number(getGridView) || 0" @click="changeViewMode('grid')"
        class=" tdprb-h-full tdprb-aspect-square tdprb-flex tdprb-justify-center tdprb-items-center tdprb-rounded-sm "
    ...
    <button v-if="Number(getListView) || 0" @click="changeViewMode('list')"
        class=" tdprb-h-full tdprb-aspect-square tdprb-flex tdprb-justify-center tdprb-items-center tdprb-rounded-sm "
...
<div class='tdprb-w-full tdprb-h-full'>
    ...
    <template v-else>
        <DiamondGridView v-show="productViewMode == 'grid'" :products="data" />
        <DiamondListView v-show="productViewMode == 'list'" :products="data" />
...
export default {
    data() {
        return {
            productViewMode: null, // grid | list
            sortByValue: 1,
    ...
    computed: {
        ...
        data() {
            return this.$store.getters['diamondListApi/data']
        },
        dataCount() {
            return this.$store.getters['diamondListApi/dataCount']
        },
        currentPage() {
            return this.$store.getters['diamondListApi/currentPage']
        },
        lastPage() {
            return this.$store.getters['diamondListApi/lastPage']
        },
        dataNotFound() {
            return this.$store.getters['diamondListApi/dataNotFound']
        },
        dataNotFoundMsg() {
            return this.$store.getters['diamondListApi/dataNotFoundMsg']
        },
        loading() {
            return this.$store.getters['diamondListApi/loading']
        },
        loadMoreLoading() {
            return this.$store.getters['diamondListApi/loadMoreLoading']
        },
        getAllFilters() {
            return this.$store.getters['diamondFilters/getAllFilters'];
        },
        diamondType() {
            return this.$store.getters['diamondFilters/getFilterValue']('diamond_type');
        },
        diamondShape() {
            return this.$store.getters['diamondFilters/getFilterValue']('shape');
        },
        sortField() {
            return this.$store.getters['diamondFilters/getFilterValue']('sort_field');
        },
        getDefaultViewMode() {
            return this.$store.getters['diamondSettings/getDefaultView'];
        },
        getGridView() {
            return this.$store.getters['diamondSettings/getGridView'];
        },
        getListView() {
            return this.$store.getters['diamondSettings/getListView'];
        },
        toSentenceCase() {
            return (value) => {
    ...
    methods: {
        ...
        callProductListApi(type = 'override') {
            this.$store.dispatch('diamondListApi/fetchDiamondList', {
